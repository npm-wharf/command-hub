...
async function handle (comhub, log, argv) {
  const {image, channel, filter} = argv
  let results
  try {
    if (channel) {
      results = await comhub.getCandidatesOnChannel(channel, image, filter)
    } else {
      results = await comhub.getCandidatesOnAll(image, filter)
    }
  } catch (err) {
    log.error(`failed to fetch candidates across clusters with: ${err.stack}`)
    return
  }
